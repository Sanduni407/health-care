...
const PaymentPage = () => {
  ...
  const handleInsurancePayment = async () => {
    if (!insuranceProvider || !policyNumber || !policyHolderName) {
      alert('Please fill in all required insurance information');
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        alert('Insurance claim submitted successfully! You will be notified once processed.');
        ...
        alert(response.data.message || 'Insurance claim submission failed');
        setProcessing(false);
      }
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="bg-white/70 backdrop-blur-sm rounded-xl shadow-lg p-6 mb-6 border-2 border-white/50">
          ...
          <div className="space-y-3">
            ...
            {/* Insurance Payment */}
            <label className={`flex items-center p-4 border-2 rounded-xl cursor-pointer transition-all duration-300 hover:shadow-md ${
              paymentMethod === 'insurance' ? 'border-teal-600 bg-teal-50/80 shadow-md' : 'border-gray-200 bg-white/50'
            }`}>
              <input
                type="radio"
                name="payment"
              ...
              <div className="flex items-center flex-1">
                ...
                <div>
                  <span className="font-bold text-gray-800 block text-lg">Insurance Coverage</span>
                  <span className="text-sm text-gray-600">Submit insurance claim</span>
                </div>
              </div>
              {paymentMethod === 'insurance' && (
                <svg className="w-6 h-6 text-teal-600" fill="currentColor" viewBox="0 0 20 20">
        ...
        {paymentMethod === 'insurance' && (
          <div className="bg-white/70 backdrop-blur-sm rounded-xl shadow-lg p-6 mb-6 border-2 border-white/50">
            <div className="flex items-center mb-5">
              ...
              <h2 className="text-xl font-bold text-gray-800">Insurance Information</h2>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent bg-white"
                  required
                >
        ...
        {paymentMethod === 'insurance' && (
          <div className="bg-purple-50/80 backdrop-blur-sm border-2 border-purple-200 rounded-xl p-5 mb-6">
            <div className="flex">
              <svg className="w-6 h-6 text-purple-600 mr-3 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <div>
                <p className="text-sm text-purple-900 font-bold mb-1">🏥 Insurance Claim Submission</p>
                <p className="text-sm text-purple-700">Your insurance claim will be submitted for verification. You'll be notified once the claim is processed by your insurance provider (typically 3-5 business days).</p>
              </div>
            </div>
          </div>
        )}
